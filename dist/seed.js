...
async function seed() {
    ...
    const hobbies = await models_1.Hobby.bulkCreate(hobbyData.map(name => ({ name })));
    ...
    const password = await (0, helpers_1.hashPassword)('testpass123');
    const users = await Promise.all(usersData.map(u => models_1.User.create({ ...u, password, isActive: true, onboardingComplete: true })));
    console.log(`Created ${users.length} users`);
    // Make all users friends (complete graph)
    for (let i = 0; i < users.length; i++) {
        for (let j = i + 1; j < users.length; j++) {
            await models_1.Friend.bulkCreate([
    ...
    for (let i = 0; i < users.length; i++) {
        for (let j = i + 1; j < users.length; j++) {
            await models_1.FriendRequest.create({ fromUserId: users[i].id, toUserId: users[j].id, status: 'accepted' });
        }
    }
    ...
    for (let i = 0; i < postContents.length; i++) {
        ...
        for (let p = 0; p < pc.photos.length; p++) {
            await models_1.PostPhoto.create({ postId: post.id, image: pc.photos[p], order: p });
        }
        posts.push(post);
    }
    ...
    for (const post of posts) {
        ...
        for (const liker of likers) {
            await models_1.PostLike.findOrCreate({ where: { userId: liker.id, postId: post.id }, defaults: { userId: liker.id, postId: post.id, likeType: 'like' } });
        }
    }
    ...
    for (const post of posts) {
        ...
        for (const commenter of commenters) {
            ...
            await models_1.Comment.create({ postId: post.id, userId: commenter.id, text });
        }
    }
    ...
    for (const [u1, u2] of convPairs) {
        ...
        await conv.setParticipants([u1.id, u2.id]);
        ...
        for (let m = 0; m < numMessages; m++) {
            ...
            if (m === numMessages - 1) {
                ...
                await models_1.ConversationReadStatus.upsert({
                    conversationId: conv.id,
                    userId: receiver.id,
